---
title: "개인정보처리방침 — 틈트니 (영문명: FitFlow)"
---
...
본 애플리케이션(“틈트니(FitFlow)”, “본 앱”)은 서비스 제공 및 계정 관리를 위해 사용자의 데이터를 **직접 수집하고 FireStore 서버에 저장**합니다. 본 앱이 수집하는 주요 데이터는 **구글 계정을 통한 사용자 식별 정보(이메일 주소, 사용자 ID)** 및 **사용자가 직접 기록한 운동 데이터(피트니스 정보)**입니다.
...
본 앱 개발자는 수집된 사용자 ID 및 운동 기록을 **서비스 운영 및 개선을 목적으로 직접 관리**합니다.

---

### 1. 앱이 직접 수집·저장하는 데이터

본 앱은 서비스 제공 및 계정 관리를 위해 다음 데이터를 **직접 수집하고 FireStore 서버에 저장**합니다.
...
본 앱 개발자는 수집된 사용자 계정 정보(이메일, ID) 및 모든 스쿼트 기록을 **FireStore 서버에 보관**합니다.
...
FitFlow (“the App”) **directly collects and stores** user data on the **FireStore server** for the purpose of service provision and account management. The key data collected includes **user identification information (Email address, User ID) via Google Sign-In** and **user-recorded workout data (Fitness Information)**.
...
The App developer directly manages and accesses the collected User IDs and workout records solely for the purpose of operating and improving the service.

---

### 1. Data the App Collects Directly

The App directly collects and stores the following data on the FireStore server for service provision and account management:
...
The App developer retains the collected **user account information (email, ID) and all squat records on the FireStore server.**
